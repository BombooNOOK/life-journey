...
import { BookshelfViewportSwitcher } from "@/components/orders/BookshelfViewportSwitcher";
...
import {
  combinePdfDownloadLimit,
  fetchAccountPdfDownloadLimitOrNull,
} from "@/lib/order/effectivePdfDownloadLimit";
...
export default async function BookshelfPage() {
  ...
  try {
    ...
    const deployRevision = process.env.VERCEL_GIT_COMMIT_SHA?.slice(0, 7) ?? null;
    ...
    const forestKanteiBooks = orders.map((order) => {
    ...
    const forestDiaryBooks = diaryBookRows.map((book) => {
    ...
    const mobile = (
      <ForestBookshelfClient
        ...
        kanteiBooks={forestKanteiBooks}
        diaryBooks={forestDiaryBooks}
        blockCreate={!canUseContinuedFeatures}
        deployRevision={deployRevision}
      />
    );

    return <BookshelfViewportSwitcher mobile={mobile} desktop={desktop} />;
  } catch (e) {
