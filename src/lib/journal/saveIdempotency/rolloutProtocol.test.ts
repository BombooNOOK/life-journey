...
describe("4B-4AI-1 capability admission", () => {
  it.each([
    ...
    expect(
      isJournalSaveIdempotencyRolloutEligible({ globalEnabled, rollout }),
    ).toBe(expected);
...
describe("4B-4AI-6.3a authoritative admission resolver", () => {
  it("uses the same predicate and fails closed when the rollout read fails", async () => {
    await expect(
      resolveJournalSaveIdempotencyRolloutEligibility({
        globalEnabled: true,
        actorKey: "actor@ljd.invalid",
        loadRollout: async () => ({ enabled: true, protocolVersion: 1 }),
      }),
    ).resolves.toBe(true);
    await expect(
      resolveJournalSaveIdempotencyRolloutEligibility({
