...
describe("compositeDailyNumberCarousel", () => {
  it(
    "generates 9 slides including closing for UD8 owl payload",
    async () => {
  ...
  it(
    "output matches template dimensions",
    async () => {
      const resolved = resolveDailyNumberPost({
        scheduledDate: "2026-06-19",
        todayNumber: 8,
  ...
  it(
    "cover slide reflects scheduled date overlay",
    async () => {
      const base = resolveDailyNumberPost({
        scheduledDate: "2026-06-19",
        todayNumber: 8,
        character: "owl",
        messageType: "base",
      });
      const other = resolveDailyNumberPost({
        scheduledDate: "2026-07-15",
        todayNumber: 4,
        character: "owl",
        messageType: "base",
      });
      if (!base.ok || !other.ok) throw new Error("expected ok");

      const [coverA, coverB] = await Promise.all([
        compositeDailyNumberCarousel(base.payload).then((s) => s[0]!.buffer),
        compositeDailyNumberCarousel(other.payload).then((s) => s[0]!.buffer),
      ]);
      expect(coverA.equals(coverB)).toBe(false);
    },
    30_000,
  );

  it(
    "ZIP に instagram-caption.txt を含む",
    async () => {
      const resolved = resolveDailyNumberPost({
        scheduledDate: "2026-06-19",
        todayNumber: 8,
