...
export type MoriLogBgmPickerHandle = {
  stopPreview: () => void;
};
...
type Props = {
  ...
  /** 省略時は日記由来ムービー用の音楽堂カタログ */
  tracks?: readonly MoriLogBgmTrack[];
  heading?: string;
  hint?: string;
};

/** 森ログムービー用：BGM一覧・試聴・選択（MP4書き出しは別ステップ） */
export const MoriLogBgmPicker = forwardRef<MoriLogBgmPickerHandle, Props>(
  function MoriLogBgmPicker(
    {
      value,
      onChange,
      tracks = MORI_LOG_BGM_TRACKS,
      heading = "ムービーのBGM",
      hint = "森の小さな音楽堂の曲から選べます。▶ で試聴できます（この段階では動画の書き出しはありません）。",
    },
    ref,
  ) {
    ...
    useImperativeHandle(ref, () => ({ stopPreview }), [stopPreview]);
    ...
    return (
      <div className="space-y-3">
        <div>
          <p className="text-sm font-medium text-[#3f3428]">{heading}</p>
          <p className="mt-1 text-xs leading-relaxed text-[#6b5a48]">{hint}</p>
        </div>
              ...
              <li key={track.id}>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    >
                      <span className="block text-sm font-semibold text-[#3f3428]">
                        {track.title}
                      </span>
                    ...
                    <button
                      ...
                      aria-label={
                        playing ? `${track.title}を停止` : `${track.title}を試聴`
                      }
                      className={[
