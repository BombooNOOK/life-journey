...
/** 冒頭・締め用：全身イラスト（やや大きめ） */
function FullCharacterIcon({ name, className }: { name: DecorationName; className?: string }) {
  return (
    <DecorationImage
      name={name}
      size="md"
      ...
      fallback={
        <span
          aria-hidden
          className={["inline-block h-11 w-9 shrink-0 rounded-full bg-stone-100/90", className]
            .filter(Boolean)
            .join(" ")}
        />
      }
...
export function HomeRecommendedForSection() {
  return (
    <section className="rounded-2xl border border-stone-200/75 bg-[#fdfaf4] p-4 shadow-sm sm:p-5">
      ...
      <ConversationDivider />
      <ClosingLine>{CLOSING_MESSAGE}</ClosingLine>
    </section>
  );
}
