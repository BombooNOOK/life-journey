...
import {
  ...
  resolveApprovedProductionMigrationTarget,
...
export function runControlledMigration(options) {
  ...
  const targetResult = resolveApprovedProductionMigrationTarget(
    options.targetMigrationId ?? env[CONTROLLED_MIGRATE_TARGET_ENV],
  );
  if (!targetResult.ok) {
    log(`[controlled-migrate] BLOCK ${targetResult.code}: ${targetResult.detail}`);
    return {
      ok: false,
      phase: "target",
  ...
  const sqlPath = path.join(process.cwd(), target.sqlRelativePath);
  if (!fs.existsSync(sqlPath)) {
    log(`[controlled-migrate] BLOCK target_sql_review_missing: ${target.id}`);
    return {
      ok: false,
      phase: "target",
      code: "target_sql_review_missing",
      detail: "approved target SQL is absent; refuse migration",
    };
  }
  ...
  if (expectedPending !== target.id) {
    log(
      `[controlled-migrate] BLOCK target_pending_binding_mismatch: target and expected pending must be the same singleton`,
    );
    return {
      ok: false,
      phase: "pending",
      code: "target_pending_binding_mismatch",
      detail: "expected pending list must equal the approved target migration exactly",
      target: target.id,
    };
  }
  ...
  log(`[controlled-migrate] sql_review_path=${target.sqlRelativePath}`);
