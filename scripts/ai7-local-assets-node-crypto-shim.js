/**
 * Local-assets IIFE shim for `node:crypto`.
 *
 * Capacitor WebView bundles cannot import Node builtins. The native harness
 * only needs sync SHA-256 (`createHash("sha256")`) for request fingerprints.
 * Other Node crypto APIs throw so they cannot silently enter the device bundle.
 *
 * Not used by Production Next / Vercel builds.
 */
...
export function createHash(algorithm) {
  ...
  return {
    ...
    digest(encoding) {
      ...
      if (encoding === "hex" || encoding === undefined) return bytesToHex(digest);
